...
## 2026-06-16 — Batch 3 validation

**Decision**: Add isolated Scenario A fixture (`fixtures/os-scenario-a-fullstack/`) with subagent orchestration log.
**Why**: Close A-1 NOT RUN gap without touching production site.

**Decision**: Local Playwright + `screenshot-qa` skill (not rejected upstream candidate).
**Why**: Deterministic 5-viewport baselines; never auto-update on failure.

**Decision**: `tools/web_content_safety/scanner.py` with 18 injection fixtures.
**Why**: Move web gate from PASS_WITH_LIMITATIONS to automated PASS.
